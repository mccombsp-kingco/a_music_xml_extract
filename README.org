...
** Parse XML
   ...
   SongIDs for values.
** Demo
   Increasingly misnamed, As of early January 2026, demo.py imports library_parse_mod.py, then presents the user with a menu of choices. The menu allows for a case insensitive search on Artist, Song Title, and Album title that returns an unsorted brief table of song attributes, a raw dump of an entire song record from the appropriate XML tag, and allows the user to quit the application. The brief table includes: song key, song title, artist name, album name, and  date and time last played. The last is currently hard coded to return US Pacific time.
** Flask Toy
   flask_toy.py implements the Flask web application framework to run on my local machine and provide a web interface. To start with there is no user interaction. It imports demo.py queries for songs with "Giants" in the artist key, sets a non-functional menu, and displays the query results in a non-wrapping HTML ~<pre>~ tag.

   To get my non-interactive page I need to create the following:
   1. flask_toy.py
   2. ./templates/index.html
   3. ./static/flask.css

   Start the webserver with default settings by running flask_toy from the commandline in its directory ~python3 flask_toy.py~
* Dependencies
** [[https://pandas.pydata.org/][Pandas Data Library for Python]]
   + Using Pandas to display search results with a reasonably tidy presentation. Currently it is set to wrap lines rather than elide data from the table to make it fit.
   + Currently I'm suppressing a FUTUREWARNING regarding the concat function and how it treats empty data frames. I don't think it will be a problem when the behavior changes in a future version.

** Match-Case syntax
   Introduced in Python 3.10 in 2021. Not terribly new, but there are plenty of dinosaurs out there, and this documentation is mostly just for future me.

** [[https://flask.palletsprojects.com/en/stable/][Flask web application framework]]
